 ...
 *******************************************************************************/
package org.zusecoin.security.session;

import java.util.Date;

import org.zusecoin.security.auth.AuthResponseCodes;
import org.zusecoin.security.auth.AuthenticationException;
import org.zusecoin.security.auth.TempToken;
import org.zusecoin.storage.StorageException;
import org.zusecoin.storage.StorageService;
...
public class SessionManager {
	 ...
	 */
	public void authenticateSession(String token, String publicKey) throws AuthenticationException {
		if (Strings.isNullOrEmpty(publicKey) || Strings.isNullOrEmpty(token)) {
			throw new AuthenticationException(AuthResponseCodes.INVALID_TOKEN,
		...
		try {
			...
			if (!session.userId.equals(publicKey)) {
				throw new AuthenticationException(AuthResponseCodes.INVALID_TOKEN, "Invalid token");
			}
		} catch (StorageException e) {
			e.printStackTrace();
			throw new AuthenticationException(AuthResponseCodes.INVALID_ADDRESS,
